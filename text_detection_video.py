...
def decode_predictions(scores, geometry):
	...
	for y in range(0, numRows):
		...
		for x in range(0, numCols):
			...
			# we compute the offset factor as our resulting feature
			# maps will be 4x smaller than the input image
			(offsetX, offsetY) = (x * 4.0, y * 4.0)

			# we extract the rotation angle for the prediction and
			# then compute the sin and cosine
			angle = anglesData[x]
			cos = np.cos(angle)
			sin = np.sin(angle)

			# we use the geometry volume to derive the width and height
			# of the bounding box
			h = xData0[x] + xData2[x]
			w = xData1[x] + xData3[x]

			# we compute both the starting and ending (x, y)-coordinates
...
while True:
	...
	# we decode the predictions, then  apply non-maxima suppression to
